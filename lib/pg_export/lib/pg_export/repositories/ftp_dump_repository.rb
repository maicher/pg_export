...
class PgExport
  module Repositories
    class FtpDumpRepository
      def all(database_name:, ftp_gateway:)
        ftp_gateway.list([database_name, '*'].compact.join('_')).map do |name:, size:|
          begin
      ...
      def by_database_name(database_name:, ftp_gateway:, offset:)
        ftp_gateway.list(database_name + '_*').drop(offset).map do |item|
          begin
