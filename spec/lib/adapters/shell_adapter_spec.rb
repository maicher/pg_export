...
RSpec.describe PgExport::Adapters::ShellAdapter do
  ...
  let(:shell_adapter) { described_class.new }
  ...
  describe '#pg_restore' do
    ...
    before(:each) do
      ...
      c.close
      shell_adapter.pg_dump(file, database)
    end
  ...
  describe '#pg_dump' do
    subject { shell_adapter.pg_dump(file, database) }
